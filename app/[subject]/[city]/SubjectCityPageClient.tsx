    ...
    <div className="min-h-screen bg-white">
        ...
        <div className="relative max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <button href="/mock-exams" className="group inline-flex items-center gap-3 px-8 py-4 bg-white text-slate-900 font-black text-lg rounded-2xl shadow-2xl hover:scale-105 transition-all">
                Start free practice
              ...
              <div className="mt-5 flex flex-wrap gap-4 text-sm font-semibold text-slate-400">
                <span className="flex items-center gap-1.5"><Check size={13} className="text-emerald-400" /> Free access</span>
                <span className="flex items-center gap-1.5"><Check size={13} className="text-emerald-400" /> Downloadable papers</span>
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3, duration: 0.8 }} className="hidden lg:block">
              <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-3xl p-6 space-y-4">
                ...
                <div className="grid grid-cols-3 gap-3">
                  {[{ val: '87%', label: 'Accuracy' }, { val: '12', label: 'Days active' }, { val: '+240', label: 'Points earned' }].map((s) => (
      ...
      <div className="bg-slate-50 border-y border-slate-200 py-5 px-4">
        <div className="max-w-5xl mx-auto flex flex-wrap justify-center gap-8 text-sm font-bold text-slate-500">
          <span className="flex items-center gap-2"><Users size={16} className="text-indigo-400" /> Families across the UK</span>
          <span className="flex items-center gap-2"><Star size={16} className="text-amber-400 fill-amber-400" /> Parent-focused resources</span>
          <span className="flex items-center gap-2"><CheckCircle size={16} className="text-emerald-500" /> GL & CEM exam aligned</span>
          <span className="flex items-center gap-2"><Trophy size={16} className="text-violet-500" /> Extensive question bank</span>
        </div>
      </div>
      ...
      <section className="py-20 px-4 sm:px-6 bg-white">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
            {topics.map((topic, i) => (
              <motion.div key={topic} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }}   className="flex items-center gap-3 p-4 rounded-2xl bg-slate-50 border border-slate-100 hover:border-indigo-200 hover:bg-indigo-50 hover:shadow-lg transition-all group text-left">
                <Link href="/mock-exams" className="flex items-center gap-3">
      ...
      <section className="py-20 px-4 sm:px-6 bg-slate-50 border-y border-slate-200">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-12"><h2 className="text-3xl font-black text-slate-900 mb-3">How {city.label} families use 11 Plus Exam Papers</h2></div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
            <div className="hidden md:block absolute top-10 left-[17%] right-[17%] h-px bg-slate-200" />
            {[
              { step: '01', title: 'Choose a resource', desc: 'Start with mock exams, papers, and structured revision for the 11+.' },
              ...
              { step: '03', title: 'Review and improve', desc: 'See exactly which topics to focus on. Progress updates after every session.' },
            ].map((s, i) => (
      ...
      <section className="py-20 px-4 sm:px-6 bg-white">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { name: 'Sarah M.', text: `The structured practice and explanations helped us focus on the right areas. The explanations are really clear.`, school: 'Preparing for local grammar school' },
              { name: 'James T.', text: `We live in ${city.label} and were worried about the ${subject.label} paper. The structured practice made it much easier to plan preparation.`, school: 'Year 5 parent' },
      ...
      <section className="py-24 px-4 sm:px-6 bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 relative overflow-hidden">
        ...
        <div className="relative max-w-3xl mx-auto text-center">
          ...
          <p className="text-slate-300 text-xl mb-10 max-w-xl mx-auto">Free 11+ {subject.label} practice. Free access. Start in under a minute.</p>
          <Link href="/mock-exams" className="group inline-flex items-center gap-3 px-10 py-5 bg-white text-slate-900 font-black text-xl rounded-2xl shadow-2xl hover:scale-[1.03] transition-all">
            Explore mock exams
          ...
          <div className="mt-6 flex flex-wrap justify-center gap-6 text-sm font-semibold text-slate-400">
            <span className="flex items-center gap-2"><CheckCircle size={15} className="text-emerald-400" /> Free access</span>
            <span className="flex items-center gap-2"><CheckCircle size={15} className="text-emerald-400" /> Downloadable papers</span>
